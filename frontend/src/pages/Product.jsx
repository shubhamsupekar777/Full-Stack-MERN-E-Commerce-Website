...
const Product = () => {
 ...
 useEffect(()=>{
  ...
  <img  
    ...
    key={index} 
    alt='' 
    className='w-[24%] sm:w-full h-[100px] sm:h-[120px] object-cover mb-3 flex-shrink-0 cursor-pointer'
  />
))
              }
          </div>
            ...
            <img  className='w-full h-[400px] object-cover'src={image} alt="" />
              ...
              <div className='flex-1'>
                ...
                <div className='flex flex-col gap-4 my-8'>
                  ...
                  <div className='flex gap-2'>
                    {productData.sizes.map((item,index)=>(
                      <button  onClick={()=>setSize(item)}className={`py-2 px-4 border bg-gray-150 ${item==size ? 'border-orange-500':' '}`} key={index}>{item}</button>
                    ))}
                  </div>
                ...
                <div className='text-sm text-gray-500 mt-5 flex flex-col gap-1'>
                  ...
                  <p>Easy Return And Exchange Within 7 Days Policy</p>
                </div>
              </div>
